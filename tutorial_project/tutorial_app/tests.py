...
class CategoryMethodTest(TestCase):
	def test_likes_are_positive(self):
		"""
		Ensure Views are positive should result true for categories where views are zero or positive
		"""
		user = add_user()
...
class IndexViewTest(TestCase):
	def test_index_view_with_no_categories(self):
		"""
		If no categories exist, an appropiate message should be displayed.
		"""
	...
	def test_index_view_with_categories(self):
		"""
		If Categories exist an appropiate message should proceed
		"""
		user = add_user()
		add_cat(user, 'test', 1)
		add_cat(user, 'temp', 1)
